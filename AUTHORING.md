...
國文的形音義模組在 `src/content/hanzi/`（路由 `/hanzi/[id]`，掛在章節地圖
`chinese-pre-2 字音字形`）。**精熟循環**：答錯的字會在幾題後回鍋、要**連對 2 次**
才過關；「一次就對」才算實力分。兩種題型：
...
3. 檔名一天一檔（例 `week3-day1.ts`），export 一個 `HanziSet`，
   到 `src/content/hanzi/index.ts` 註冊。`id` 例 `hanzi-w3-d1`，存過紀錄就別改。
